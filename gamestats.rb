...
def gamestats(filename = 'gameresults.json')
  ...
  playernames = players.uniq.each { |player| }
  ...
  players.uniq.each do |player|
    print " - #{player[0][0..player[0].length - 4]} started first: #{players.count(player)} times\n\n"
  end
  print " - #{playernames.flatten[0][0..playernames.flatten[0].length - 4]} won: #{winners.count(playernames.flatten[0]) + winners.count(playernames.flatten[3])} times\n\n"
  print " - #{playernames.flatten[1][0..playernames.flatten[1].length - 4]} won: #{winners.count(playernames.flatten[1]) + winners.count(playernames.flatten[2])} times\n\n"
  print ' - Total time taken for simulation: '
  timetaken = Time.parse(times[times.length - 1]) - Time.parse(times[0])
  print "#{(timetaken / 60).floor} mins #{timetaken % 60} seconds\n\n"
end
